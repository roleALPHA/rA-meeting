...
- User-facing texts and error messages use message IDs (`t('area.name')`, `assert(cond, 'error.area.name', status)`). Add every new ID to `shared/locales/de.ts` and to `en.ts`, `fr.ts` and `es.ts`; the compiler rejects missing translations and `tests/i18n.test.ts` rejects unused IDs. IDs ending in `@agenda` are the wording for templates set to "Agenda items"; pass the meeting's `terminologyOf(template)` as the third argument of `t()`. Stored content (event details, template text) is not translated.
...
- Visual design follows roleALPHA's corporate design (rA-app). Brand values live only in `client/brand.css`; `client/style.css` derives tokens from them. No raw colours in `style.css` or client code (`tests/brand.test.ts`, ESLint). Status colours (`--status-*`) mean state only, category tiles use data colours, selection uses `--selection`, dashed borders mean "not there yet". Every screen must work in the light, dark and contrast themes (`npm run dev`, `?theme=dark|contrast`).

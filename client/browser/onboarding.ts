...
export function workspaceUrl(value: string, current: string): string {
  ...
  try {
    ...
    throw new Error('Bitte eine gültige SharePoint-Websiteadresse eingeben.');
  }
  assert(
    url.protocol === 'https:' &&
...
export async function provisionLandingPage(host: BrowserHost, store: SharePointRestStore) {
  ...
  try {
    ...
    throw new Error('Die Einstiegsseite wurde geändert. Vorhandene Inhalte werden nicht überschrieben.');
  }
  assert(
  ...
  const checkPageUrl = (value?: string) => {
    assert(value, 'Die Einstiegsseite konnte nicht erstellt werden.');
    const actual = new URL(value, host.webUrl + '/');
    assert(actual.href === marker!.url, 'Die Einstiegsseite konnte nicht erstellt werden.');
    return actual.href;
  };
  if (
    content.length === 1 &&
...
export async function setupWorkspace(
  ...
  progress('Speicherbereiche werden eingerichtet …');
  ...
  progress('Startvorlagen werden eingerichtet …');
  await store.seed(host.tenantId, language);
  let pageUrl: string | undefined;
  if (createPage) {
    progress('Einstiegsseite wird vorbereitet …');
  ...
  assert(check.id === id, 'Der Speichertest ist fehlgeschlagen.');

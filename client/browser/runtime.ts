...
import { z } from 'zod';
import { assert, AppError, outputTypes, type Actor, type Meeting, type Template, type Tension, type Bootstrap } from '../../shared/model';
...
import { addOutcome, command, createMeeting, event, getMeeting, saveMeeting, saveTemplate, setTranscript } from '../../shared/domain';
...
export async function createBrowserApi(host:BrowserHost):Promise<AppApi>{
 ...
 const actor:Actor={id:z.string().uuid().parse(host.userId),name:host.userName,tenantId:host.tenantId,admin:access.write,workspace:access.write?'write':'read'};
...
export function browserApi(host:BrowserHost,store:Repository,actor:Actor):AppApi{
 ...
 const verify=(m:Meeting,body:Record<string,unknown>)=>assert(m.revision===rev.parse(body.revision),'Meeting wurde inzwischen geändert. Bitte neu laden.',409);
 ...
 async function transcript(m:Meeting){
  ...
  if(!id&&m.calendar?.joinUrl){
   ...
   assert(meetings.value.length===1,'Meeting ist nicht mit Teams verknüpft.');id=meetings.value[0].id;
  }
  assert(id,'Meeting ist nicht mit Teams verknüpft.');
  const prefix=`/me/onlineMeetings/${encodeURIComponent(id)}/transcripts`;
  let path:string|undefined=prefix;const parts:{id:string;createdDateTime:string}[]=[];const pages=new Set<string>();
  while(path){assert(!pages.has(path)&&pages.size<20,'Ungültige Graph-Folgeseite.');pages.add(path);const result:{value:typeof parts;'@odata.nextLink'?:string}=await(await read(path)).json();parts.push(...result.value);assert(parts.length<=100,'Zu viele Transkriptteile.');if(!result['@odata.nextLink'])break;const u=new URL(result['@odata.nextLink']);assert(u.origin==='https://graph.microsoft.com'&&u.pathname===`/v1.0${prefix}`,'Ungültige Graph-Folgeseite.');path=u.pathname.slice(5)+u.search;}
  assert(parts.length,'Noch kein Transkript verfügbar.');let raw='';
  for(const part of parts.sort((a,b)=>a.createdDateTime.localeCompare(b.createdDateTime))){raw+='\n\n'+await(await read(`${prefix}/${encodeURIComponent(part.id)}/content?$format=text/vtt`)).text();assert(raw.length<=1_000_000,'Transkript ist zu groß (max. 1 MB).',413);}
 ...
 async function dispatch(path:string,raw?:unknown,method='POST'):Promise<unknown>{
  ...
  switch(action){
   case 'command':command(m,actor,body);return save(m);
   case 'assist':return {revision:m.revision,suggestion:await assistBrowser(host,m,assistanceInput.parse(body.input))};
   case 'transcript':return setTranscript(m,actor,parseTranscript(z.string().max(1_000_000).parse(body.text)))?save(m):m;
   case 'analyze':{const results=await analyzeBrowser(host,m,parseLanguage(typeof body.language==='string'?body.language:host.settings.language));for(const output of results)addOutcome(m,actor,output,'ai');m.analyzedHash=m.transcriptHash;event(m,actor,'analysis.completed',`${results.length} Ergebnisvorschläge extrahiert.`);return save(m);}
   case 'calendar-link':{
    assert(m.status!=='completed','Abgeschlossene Meetings behalten ihre Terminzuordnung.');assert(body.organizerId===actor.id,'Kein Zugriff auf diesen Kalender.',403);
    const eventId=z.string().min(1).max(2000).parse(body.eventId);assert(!m.calendar||(m.calendar.organizerId===actor.id&&m.calendar.eventId===eventId),'Bestehende Terminzuordnung kann nicht umgebogen werden.',409);
    const linked=await calendarEntry(actor.id,eventId,read);assert(!linked.cancelled||m.calendar,'Ein abgesagter Termin kann nicht neu verbunden werden.');
    const others=await store.list<Meeting>(actor.tenantId,'meeting');assert(!others.some(o=>o.id!==m.id&&o.calendar?.organizerId===actor.id&&o.calendar.eventId===eventId),'Dieser Termin ist bereits mit einem Meeting verbunden.',409);
   ...
   case 'export':case 'export-entity':{
    ...
    if(action==='export-entity')assert(JSON.stringify(body.plan)===JSON.stringify(plan),'Exportvorschau hat sich geändert. Erneut prüfen.',409);
    const target=host.settings.roleAlpha;assert(target,'roleALPHA ist nicht verbunden.',503);
    const connection=await prepareExport(host,plan,target);
    try{
     outputs.forEach(o=>{o.export={state:'sending',startedAt:new Date().toISOString()};});event(m,actor,'export.started',plan.label);await save(m);
     try{const receipt=await connection.send();const current=await get(id,true);current.outcomes.filter(o=>ids.includes(o.id)).forEach(o=>{o.export={state:'draft_created',draftId:receipt.draftId,entityUuid:receipt.entityUuid};});event(current,actor,'export.completed',receipt.draftId);return await save(current);}
     catch{const current=await get(id,true);current.outcomes.filter(o=>ids.includes(o.id)).forEach(o=>{o.export={state:'uncertain'};});event(current,actor,'export.uncertain','Exportantwort unklar; automatische Wiederholung gesperrt.');await save(current);throw new AppError(502,'Export nicht eindeutig bestätigt. Prüfe den Entwurfsbereich in roleALPHA.');}
    }finally{await connection.close().catch(()=>{});}
   }
   case 'export-reconcile':{
    const ids=z.array(z.string().uuid()).min(1).parse(body.ids);const note=z.string().trim().min(10).max(1000).parse(body.note);const resolution=z.enum(['created','not-created']).parse(body.resolution);
    ...
    outputs.forEach(o=>{o.export=resolution==='created'?{state:'draft_created',draftId:z.string().min(1).max(200).parse(body.draftId)}:undefined;});event(m,actor,'export.reconciled',`${resolution}: ${note}`);return save(m);
   }

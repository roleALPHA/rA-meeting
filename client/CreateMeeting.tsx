...
export function CreateMeeting({
  ...
  return (
    <Modal title={tr('Meeting anlegen')} close={close}>
      <form
        onSubmit={e => {
          e.preventDefault();
      ...
      >
        <Field label={tr('Meetingname')}>
          <input
            required
            value={title}
            onChange={e => setTitle(e.target.value)}
            placeholder={tr('z. B. Produktkreis · Wöchentliches Tactical')}
          />
        </Field>
        <Field label={tr('Kreis / Team')}>
          <input required value={circle} onChange={e => setCircle(e.target.value)} placeholder={tr('Produktkreis')} />
        </Field>
        <Field label={tr('Meeting-Template')}>
        ...
        <Field label={tr('Termin (optional)')}>

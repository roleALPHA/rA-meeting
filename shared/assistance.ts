...
export const assistanceInput = z.object({
  mode: z.enum(['proposal', 'integration']), agendaId: z.string().uuid(),
...
export const assistanceResult = z.object({
  proposal: z.string().trim().min(1).max(12000),
  rationale: z.string().max(4000),
  questions: z.array(z.string().max(1000)).max(8),
  objectionResponses: z.array(z.object({ objection: z.string().max(2000), suggestion: z.string().max(3000) }).strict()).max(20),
}).strict();

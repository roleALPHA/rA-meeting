...
export function event(m: Meeting, actor: Actor, type: string, detail: string) {
  m.events.push({ id: randomUUID(), at: now(), actor: actor.name, type, detail });
}
...
export async function saveMeeting(store: Store, actor: Actor, m: Meeting, revision: number) {
  ...
  m.updatedAt = now();
  await store.save(actor.tenantId, 'meeting', m.id, m.revision, m, revision);
  return m;
}
export async function saveTemplate(store: Store, actor: Actor, body: unknown, id?: string, version?: number) {
  ...
  await store.save(actor.tenantId, 'template', t.id, t.version, t, existing?.version);
  return t;
}
export const createMeetingInput = z.object({
  templateId: z.string().uuid(),
  title: z.string().trim().min(1).max(200),
  circle: z.string().trim().min(1).max(200),
...
export async function createMeeting(store: Store, actor: Actor, body: unknown) {
  ...
  await store.save(actor.tenantId, 'meeting', m.id, 1, m);
  return m;
}
...
export function setTranscript(m: Meeting, actor: Actor, segments: Segment[]) {
  ...
  event(m, actor, 'transcript.imported', `${segments.length} Transkriptsegmente importiert.`);
  return true;
}
  ...
  switch (type) {
    ...
    case 'skip': {
      ...
      if (m.currentStep === m.template.steps.length - 1) {
        m.status = 'completed';
        m.stepStartedAt = null;
        event(m, actor, 'completed', 'Meeting abgeschlossen.');
      } else {
        m.currentStep++;
        m.stepStartedAt = now();
      }
    ...
    case 'note':
      m.notes[step.id] = z.string().max(20000).parse(body.text);
      event(m, actor, 'note.updated', step.title);
      break;
    case 'agenda.add': {
      ...
      m.agenda.push({
        id: randomUUID(),
        stepId,
        title: z.string().trim().min(1).max(500).parse(body.title),
        owner: z
          .string()
          .max(200)
          .parse(body.owner ?? ''),
    ...
    case 'agenda.proposal': {
      ...
      a.objections = z
        .string()
        .max(12000)
        .parse(body.objections ?? '');
      event(m, actor, 'agenda.proposal.saved', `Vorschlagsentwurf gespeichert: ${a.title}`);
      break;
    }
    case 'agenda.phase':
    case 'agenda.resolve': {
      ...
      if (type === 'agenda.phase') {
        const p = z
          ...
          .int()
          .min(0)
          .max(Math.max(0, step.phases.length - 1))
          .parse(body.phase);
        a.phase = p;
        event(m, actor, type, `${a.title}: ${step.phases[p]}`);
      } else {
        a.status = 'resolved';
        event(m, actor, type, a.title);
      }
      break;
    }
    case 'outcome.add':
      addOutcome(m, actor, body.outcome);
      break;
    case 'outcome.review': {
      ...
      o.approvedAt = status === 'approved' ? now() : undefined;
      event(m, actor, `outcome.${status}`, o.title);
      break;
    }
    case 'outcome.edit': {
      const o = m.outcomes.find(o => o.id === body.id);
      ...
      Object.assign(o, input, { status: 'proposed', approvedBy: undefined, approvedAt: undefined });
      event(m, actor, 'outcome.edited', o.title);
      break;
    }
    default:

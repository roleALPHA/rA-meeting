...
const fontFiles = [...(await readFile('client/fonts.css', 'utf8')).matchAll(/url\('([^']+\.woff2)'\)/g)].map(m => m[1]);
await rm('spfx/src/generated/fonts', { recursive: true, force: true });
await mkdir('spfx/src/generated/fonts', { recursive: true });
for (const file of fontFiles) await copyFile(`client/assets/fonts/${file}`, `spfx/src/generated/fonts/${file}`);
await writeFile(
  'spfx/src/generated/fonts.ts',
  `/* eslint-disable */\n// Generated by scripts/build-browser.ts.\nexport const fonts: Record<string, string> = {\n${fontFiles
    .map(file => `  ${JSON.stringify(file)}: require(${JSON.stringify('./fonts/' + file)}),`)
    .join('\n')}\n};\n`,
);

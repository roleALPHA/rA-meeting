...
try {
  ...
  throw new Error('SPFx build dependencies missing. Run npm run spfx:install first.');
}
for (const args of [
  ['build', '--clean', '--production'],
  ['package-solution', '--production'],
]) {
  const result = spawnSync(node, ['node_modules/@rushstack/heft/lib/start.js', ...args], {
    cwd: resolve('spfx'),
    stdio: 'inherit',
...
await copyFile(
  resolve(root, 'spfx/sharepoint/solution/rolealpha-meetings.sppkg'),
  resolve(root, 'dist/rolealpha-meetings.sppkg'),
);

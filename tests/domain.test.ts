...
test('template snapshots survive edits, disabling and deletion', async () => {
  ...
  const edited = await saveTemplate(
    store,
    actor,
    { ...template, name: 'Changed', enabled: false, steps: [...template.steps].reverse() },
    template.id,
    template.version,
  );
  assert.equal(edited.version, 2);
  assert.equal((await getMeeting(store, actor, meeting.id)).template.name, 'Tactical Meeting');
  await assert.rejects(
    createMeeting(store, actor, { templateId: template.id, title: 'No', circle: 'No' }),
    /deaktiviert/,
  );
  await store.delete(actor.tenantId, 'template', template.id, 2);
  await store.seed(actor.tenantId);
  assert.equal((await store.list(actor.tenantId, 'template')).length, 2);
  assert.equal((await getMeeting(store, actor, meeting.id)).template.steps[0].kind, 'check-in');
});
test('optimistic concurrency prevents overwriting meeting and template changes', async () => {
  ...
  const second = structuredClone(meeting);
  command(meeting, actor, { type: 'start' });
  await saveMeeting(store, actor, meeting, 1);
...
test('step output allowlists and active phase boundaries cannot be bypassed', async () => {
  ...
  assert.throws(
    () => addOutcome(meeting, actor, { stepId: agenda.id, title: 'No', type: 'governance' }),
    /nicht erlaubt/,
  );
...
test('VTT import preserves speaker and source timestamps; repeated import is idempotent', async () => {
  ...
  assert.equal(segments[0].speaker, 'Anna');
  assert.equal(segments[1].text, 'Danke & bis morgen.');
  assert.equal(setTranscript(meeting, actor, segments), true);
...
test('AI results need real references and cannot invent target UUIDs or allowed outputs', async () => {
  ...
  const outcome = {
    ...
    evidence: ['s1'],
    targetId: 'f2856428-f2f8-4d72-b74d-8a3d1a970765',
  };
  ...
  addOutcome(meeting, actor, outcome, 'ai');
  command(meeting, actor, { type: 'outcome.review', id: meeting.outcomes[0].id, status: 'approved' });
...
test('editing invalidates approval and exported results cannot be edited', async () => {
  ...
  assert.equal(outcome.approvedBy, actor.id);
  command(meeting, actor, { type: 'outcome.edit', id: outcome.id, outcome: { ...outcome, title: 'Changed' } });
  assert.equal(outcome.status, 'proposed');
  assert.equal(outcome.approvedBy, undefined);
  command(meeting, actor, { type: 'outcome.review', id: outcome.id, status: 'approved' });
  const payload = exportPayload(meeting, [outcome]);
  assert.equal(payload.data.outcomes[0].title, 'Changed');

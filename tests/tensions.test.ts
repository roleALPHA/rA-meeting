...
test('tensions live independently; resolving an agenda does not resolve the tension', async () => {
  const store = new TestStore('tenant'); await store.initialize(); await store.seed('tenant');
  ...
  await assert.rejects(saveTension(store, { ...actor, id: 'stranger', admin: false,workspace:'read' }, tension, tension.id, 1), /Berechtigung/);
  ...
  await assert.rejects(attachTension(store, { ...actor, tenantId: 'other' }, tension.id, second.id, step.id, 1), /Mandant/);
...
test('workspace readers cannot contribute tensions or attach them to meetings',async()=>{
 ...
 await assert.rejects(attachTension(store,reader,tension.id,meeting.id,template.steps[2].id,meeting.revision),/Berechtigung/);
});
